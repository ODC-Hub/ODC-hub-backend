...
public class ResourceService {
    ...
    public void validateResource(String resourceId, User validator) {
        ...
        // 🔔 Homework becomes visible NOW → notify assigned users
    ...
    public void deleteResource(String id) {
        ...
        if (resource.getGridFsFileId() != null) {
            try {
                ...
                log.warn(
                        "Failed to delete GridFS file {} for resource {}",
                        resource.getGridFsFileId(),
                        id
                );
            }
        }
    ...
    private List<ResourceResponse> filterResourcesByUser(
        ...
        if (user.getRole() == Role.BOOTCAMPER) {
            return resources.stream()
                    .filter(resource -> {
                        if (resource.getType() == ResourceType.HOMEWORK) {
                            return resource.getAssignedTo() != null
                                    && resource.getAssignedTo().contains(user.getId());
                        }
                        return true;
                    })
